...
Experiment 10 provides an  report on the "goat" dataset, using an approach that equivalent to the approach of Baumen et al in the 2011 Ontario study, with some slight modifications:
...
* Prior for herd level prevelence was initiall taken as $Beta(1.55,2.66)$, but is modified below.  

**The overall intent of this experiment is to produce a set of plots to support reporting required for the grant.**
...
data5= list(F=27,
            ...
            I_e=goat_sum$i_e,
            J=3,
            y=as.matrix(goat %>% select(elisa:culture)),
...
```{r echo=FALSE}
g2=goat %>% group_by(farm_number) %>% mutate(goat_num=row_number()) %>%
...
p1a=g2 %>% ggplot(aes(x=goat_num,y=fct_inorder(test),fill=result)) +
     facet_wrap(~fct_reorder(as.factor(farm_number),median_HIP,.desc=TRUE),
                ncol=4,strip.position="left",dir="v")+
     geom_tile(color="grey", show.legend = FALSE,width=.9)+
...
doc=read_pptx()
doc=add_slide(doc)
doc=doc %>% ph_with("Results of MAP testing on 316 goats, 27 farms", 
                    ph_location_type(type="title"))
doc=doc %>% ph_with(dml(ggobj=p1a), 
...
g2_cross=g2 %>% tabyl(sex,result) %>% rename(neg="FALSE",pos="TRUE") %>%
  ...
  adorn_ns() 
g2xt=g2_cross %>% flextable()
     
```

```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("Male goats (black outline) had higher rate of positive tests.", 
                    ph_location_type(type="title"))
loc1=ph_location(left=.5,top=2,width=7,height=4)
loc2=ph_location(left=5,top=6.5,width=4,height=1)
doc=doc %>% ph_with(dml(ggobj=p1g), loc1)
doc=doc %>% ph_with(g2xt, loc2)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
```{r include=FALSE}
text1=fpar(ftext("In Bayesian statistics, the key idea is that for unknown quantities, the analysis tries to capture the uncertainty not by giving a single value for those unknown quantities, but by showing a distribution (or histogram plot) of values that arise from repeated simulation.   The values that result more often in the simulations are plotted with a higher likelihood.  By plotting the whole distribution, we get an better understanding of not only what are the likely values, but what is the range of reasonable values.  Key quantities from that analysis are plotted below.",prop10))
```
...
```{r include=FALSE}
text2=block_list(
     fpar(ftext("Herd True Prevalence (HTP) describes the fraction of herds that have at least one case of MAP infection/Johnes in the herd.   HTP could be directly measured if:",prop10)),
     fpar(ftext(" ",prop10)),
     fpar(ftext(" - Testing were perfect.",prop10)),
     fpar(ftext(" - All farms were tested.",prop10)),
     fpar(ftext(" - All goats on the farm were tested.",prop10)),
     fpar(ftext(" ",prop10)),
     fpar(ftext("This parameter is estimated from the data based on a standard model.",prop10)))
```


```{r echo=FALSE}
plot_title2 <- ggtitle("Herd Level Prevalence",
                      "with medians and 80% intervals")
phlp=mcmc_areas(posterior,prob=.8,pars="HTP") + plot_title2
...
ft2=fit5 %>% gather_draws(HTP) %>% median_hdi(.width=.9) %>% flextable()
```

```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("We use Bayesian statistics to analyze Herd True Prevalence (HTP)", 
                    ph_location_type(type="title"))
loc1=ph_location(left=.2,top=2,width=2.5,height=5.5)
loc2=ph_location(left=3,top=1.5,width=6,height=2)
loc3=ph_location(left=4,top=3.3,width=4,height=3.2)
loc4=ph_location(left=3,top=6.75,width=6.6,height=.75)

doc=doc %>% ph_with(text1, loc1)
doc=doc %>% ph_with(text2, loc2)
doc=doc %>% ph_with(dml(ggobj=phlp), loc3)
doc=doc %>% ph_with(ft2, loc4)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
```{r include=FALSE}
text1=fpar(ftext("For each herd (farm), we may estimate the probability that the farm has an infected animal (within the group of animals being tested).  We call this value Herd Infection Probability (HIP) and we estimate its value separately for each farm.  As a Bayesion parameter, it is also shown as a distribution (of values).  The value for HTP reflects an overall regional average, while HIP reflects the conditions on a farm and is a result or the data collected from that farm, as adjusted for the understanding of the accuracy of the tests and the overall conditions expected in the region.",prop10))
```

```{r echo=FALSE}
farm_list=c(9,2,4,5,10,3,1)
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("We estimate Herd Infection Probability (HIP) for each farm.", 
                    ph_location_type(type="title"))
loc1=ph_location(left=.2,top=2,width=2.5,height=5.5)
loc2=ph_location(left=3,top=2,width=6,height=4)

doc=doc %>% ph_with(text1, loc1)

doc=doc %>% ph_with(dml(ggobj=hip1), loc2)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
* For farms 5, 10, 3, and 1 note that as we increase the number of goats testes, the HIP shifts to the left, and the median probability is reduced.
...
```{r include=FALSE}
text2=block_list(
     fpar(ftext("As explanation of this figure: On the left is the HIP distribution for a selection of farms, and on the right is the associated testing data, where we have chosen farms to illustrate some key ideas:",prop10)),
     fpar(ftext(" ",prop10)),
     fpar(ftext(" - On farm 9, the positive tests were all on the same sheep which essential confirms the presence of MAP, so the infection probability is higher than on farm 2, even though it has the same number and type of positive tests.",prop10)),
     fpar(ftext(" - Comparing farm 4 and farm 10, we see the single postive ELISA pushes the HIP probability distribution (and mean value) toward higher probabilities of infection.",prop10)),
     fpar(ftext(" - For farms 5, 10, 3, and 1 note that as we increase the number of goats testes, the HIP shifts to the left, and the median probability is reduced.")),
     fpar(ftext(" ",prop10)),
     fpar(ftext("Note that overall, even though no positive tests were seen on farm three, the HIP is not 0.  (The medial value on farm 3 is actually 10%).  This risk of undetected MAP persists due to the known low sensitivity of the testing procedures in an operational setting.",prop10)))
```
...
### Farm 22 shows extremely high liklihood of infection.

```{r echo=FALSE}
farm_list=c(22)
plot_title <- ggtitle("HIP distribution")
p2g122=mcmc_areas(posterior,pars=vars(param_range("HIP",farm_list)))+plot_title
p2g222=g2 %>%  filter(farm_number %in% farm_list) %>%  ggplot(aes(x=goat_num,y=fct_inorder(test),fill=result)) +
     facet_wrap(~fct_reorder(as.factor(farm_number),median_HIP,.desc=TRUE),
                ncol=1,strip.position="left",dir="v")+
     geom_tile(color="grey", show.legend = FALSE)+
     scale_fill_manual(values = CPCOLS)+
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("Farm 22 illustrates great surety of infection.", 
                    ph_location_type(type="title"))

loc2=ph_location(left=1.5,top=2,width=6,height=3)



doc=doc %>% ph_with(dml(ggobj=hip2), loc2)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("Farms may be sorted by median infection probability", 
                    ph_location_type(type="title"))
...
doc=doc %>% ph_with(dml(ggobj=hip3), ph_location_type(type="body"))
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
```{r include=FALSE}
text1=fpar(ftext("Whereas herd true prevelence (HTP) is concerned with what fraction of herds will have an infected animal, the other prevelence measure of interest is - what fraction of goats are carrying the infection?  In the typical model, the parameter of interest considers this questions on a per farm basis, and attempts to explain ... under the assumption that MAP is present in the herd, what fraction of the herd is infected?   This quantity is designated as the conditional within herd prevalence (WHP) and is estimated, as a distribution, herd by herd for each farm, based on the testing outcomes.",prop10))
```



```{r}
farm_list=c(22,24,15,8,11,1)
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("WHP describes the extent of infection for each farm.", 
                    ph_location_type(type="title"))
loc1=ph_location(left=.2,top=2,width=2.5,height=5.5)
loc2=ph_location(left=3,top=2,width=6,height=4)
...
doc=doc %>% ph_with(dml(ggobj=whp1), loc2)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```


### Median WHP for full dataset
...
g3=goat %>% group_by(farm_number) %>% mutate(goat_num=row_number()) %>%
     select(-sample_number) %>% pivot_longer(elisa:culture, names_to = "test",values_to = "result" ) %>%
     mutate(result=as.logical(result)) %>% left_join(df_jc %>% select(farm_number,median_CWHP))
whp2=g3 %>%    ggplot(aes(x=goat_num,y=fct_inorder(test),fill=result)) +
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("Farms may be sorted by median WHP", 
                    ph_location_type(type="title"))
...
doc=doc %>% ph_with(dml(ggobj=whp2), ph_location_type(type="body"))
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
Can we visualize the age of the positive goats?

```{r echo=FALSE}
my_goat = readxl::read_excel(goat_file, sheet = "Sheet1")%>%
     clean_names()%>% mutate(sex=str_to_lower(sex)) %>%
     mutate(sex=case_when(
          str_starts(sex,"f") ~ "f",
          TRUE ~ "m"
     ))%>% mutate(age=round(as.numeric(age)) %>% as.integer())%>%
     select(sample_number,sex,age,farm_number,  elisa, pcr, culture) %>%
     filter(elisa  %in% c("p", "n")) %>%
     filter(culture %in% c("p", "n")) %>%
     mutate(across(elisa:culture, ~ (.x == "p") + 0))
...
g2a=my_goat %>% group_by(farm_number) %>% mutate(goat_num=row_number()) %>%
     select(-sample_number) %>% pivot_longer(elisa:culture, names_to = "test",values_to = "result" )%>% 
...
```{r}
p_age2=g2a %>% ggplot(aes(x=as.factor(age),fill=result)) +
     facet_wrap(~result,
                ncol=1,strip.position="left",dir="v",scales = "free_y")+
     ...
     theme(strip.background = element_blank(),
           ...
           axis.title.y=element_text(color="gray72"))+
     xlab("Age")+
     ggtitle("Age structure of test results")
...
```{r include=FALSE}
text1=fpar(ftext("Although Johne's is sometimes viewed as a disease that is more likely to affect older animals, this bar chart, summarizing the data, separates between animals testing positive and those testing negative and seems to show little difference in the age structure of those two groups.  The next slide shows a detailed annotation of the data, showing the age of each animal (if known).",prop10))
```

```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("Age structure indicates no bias toward older animals.", 
                    ph_location_type(type="title"))
loc1=ph_location(left=.2,top=2,width=2.5,height=5.5)
loc2=ph_location(left=3,top=2,width=6,height=4)
...
doc=doc %>% ph_with(dml(ggobj=p_age2), loc2)
#print(doc, target = here("rmd","goat_report_001.pptx")) 
```
...
CPCOLS <- c("#FFEFDB", "#5F9EA0")
CPCOLS2 <- c("#D1D1D1", "#000000")
p_age=g2a %>% ggplot(aes(x=goat_num,y=fct_inorder(test),fill=result)) +
     facet_wrap(~fct_reorder(as.factor(farm_number),median_HIP,.desc=TRUE),
                ncol=4,strip.position="left",dir="v")+
     geom_tile(color="grey", show.legend = FALSE,width=.9)+
     scale_fill_manual(values = CPCOLS)+
...
```{r}
doc=add_slide(doc)
doc=doc %>% ph_with("MAP data, animals labeled with age in years.", 
                    ph_location_type(type="title"))

doc=doc %>% ph_with(dml(ggobj=p_age), ph_location_type(type="body"))
print(doc, target = here("rmd","goat_report_001.pptx")) 
```
