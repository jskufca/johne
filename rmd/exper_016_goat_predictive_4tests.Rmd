---
title: "Exp 16:Using stan with heirarchical model"
author: "Joe Skufca"
date: "2025-06-09"
output: 
    html_notebook:
...
Experiment 16 examines the collaborative dataset working with the Wisconsin lab.
...
#                 farm_number=f_max+1,
#                 within_farm_number=1:s,
#                 elisa_a=this_elisa,  # simulated real samples
#                 elisa_b=0,
#                 pcr=0,  # filler 
#                 culture=0,
#                 valid_elisa_a=1,
...
We use the `PriorGen` package to assist with developing hyperparameters (the mathematical description of our prior distributions).  The wrapper functions created here allow the text output from that package to be used programmatically.
...
fit5 %>% gather_draws(HTP) %>% median_hdi(.width=.9)
# %>% flextable()
```
...
mcmc_areas(posterior,regex_pars=c("spec"))+
     scale_y_discrete(labels=c("spec_a_elisa","spec_b_alisa","spec_pcr","spec_cult"))+plot_title


#ggsave(filename=here("figs","spec_goat.png"), width=7.5,height=5,units="cm"   )
```

On the other hand, the sensitivity of these test are somewhat lower.

```{r}
plot_title <- ggtitle("Sensitivy (Goats)")
mcmc_areas(posterior,regex_pars=c("sens"))+
     scale_y_discrete(labels=c("sens_a_elisa","sens_b_elisa","sens_pcr","sens_cult"))+plot_title
...
```{r eval=FALSE, include=FALSE}
df_a=goat %>% count(elisa_a,pcr,culture) %>% arrange(elisa_a,pcr,culture)
df_a %>% flextable()
```
...
```{r}
farm_list=c(9,2,15,24,12,19,11,1,23,14)
#farm_list=c(4,5,1,3)

plot_title <- ggtitle("HIP distribution")
p4g1=mcmc_areas(posterior,pars=vars(param_range("HIP",farm_list)),
                #area_method = c("equal area"))+
                area_method = c("scaled height"))+
     plot_title 



plot_title <- ggtitle("CWHP distribution")
p4g3=mcmc_areas(posterior,pars=vars(param_range("CWHP",farm_list)))+plot_title+ geom_vline(xintercept = .25)+xlim(c(0,.3))
p4g2=g2 %>%  filter(farm_number %in% farm_list) %>%  ggplot(aes(x=goat_num,y=fct_inorder(test),fill=result)) +
     facet_wrap(~fct_relevel(as.factor(farm_number),as.character(farm_list)),
                ncol=1,strip.position="left",dir="v")+
     geom_tile(color="grey", show.legend = FALSE)+
     scale_fill_manual(values = CPCOLS)+
...
p4g1+p4g2+p4g3+ 
  plot_layout(widths = c(1.5, 1.5,1.5))
```
