...
Experiment 13 provides uses the collected data from the grant funded study and
...
that likes outside the original protocol tested.   Typically, we image this will be
...
* For the "new" herd, we will use a (-1) as the indicator of a missing test result.
...
Hypothetical - test of 20 goats with ELISA, all negative, but no other tests performed.
...
```{r}
fit5=stan(file=here("src","heir_multi_test_B_OOS.stan"),data=data5,
...
g2=goat_s %>% group_by(farm_number) %>% mutate(goat_num=row_number()) %>%
